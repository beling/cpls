...
- **Smart updates:** Unchanged files are not copied again. Files not on the list are, on request, deleted from the destination folder.
...
- **Comments:** Lines beginning with a `#` are ignored. We recommend starting the profile file with a comment listing the names of supported devices, followed by comments indicating the sources of information, such as links to device manuals or manufacturers' websites.
